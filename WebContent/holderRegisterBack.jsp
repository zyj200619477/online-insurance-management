...
ResultSet rs = null;
int hid = 0;
String sql = "INSERT INTO holder (hname,hcard,hbank,hbirth,hsex,hoccu,hincome,hadd,hemail,hpolicy,hpremium,hagent,hstart,hdue,hpwd,hauth) "
        + "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
try (Connection conn = DBConnector.getConnection();
       ...
       if(rowAffected == 1)
       {
           ...
           if(rs.next())
               hid = rs.getInt(1);
           session.setAttribute("agent", hid);
   		response.sendRedirect("success.jsp?msg=sucess");
       }
